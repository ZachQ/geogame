...
/**
 * The Home tab is an in-game tab which displays user-specific information like
 * amount of money, seed, fertilizer, etc.
 */
public class HomeTabActivity extends Activity {

	/**
	 * Various stats and counts of the player's current state
	 */
	private TextView name, money, adults, labor, seedLR, seedHYC, fertilizer,
			water, grainLR, grainHYC, oxen, timerTV, turnTV;
	...
	/**
     * Updates the player's supply counts
     */
	public class updateThread extends Thread {
		...
		public void run() {
			...
			RestClient g_client = new RestClient(GeoGame.URL_GAME + "Dash/"
					+ GeoGame.currentGameId);
			RestClient m_client = new RestClient(GeoGame.URL_MARKET + "myBank/"
					+ GeoGame.currentGameId);
			RestClient cost_client = new RestClient(GeoGame.URL_MARKET + "Get/"
					+ GeoGame.currentGameId + "/bank");
			RestClient game_time = new RestClient(GeoGame.URL_TIME
					+ GeoGame.currentGameId);
			...
			try {
				...
				try {
					...
					// if status = "end" then the other variables will not be
					// returned
	...
	/**
	 * Update the UI with the information retrieved from the server
	 */
	...
	/**
	 * Update the UI (the supply counts, which could have since changed)
	 */
	@Override
	...
	/**
	 * This method converts the milliseconds into readable format
	 * 
	 * @param time
